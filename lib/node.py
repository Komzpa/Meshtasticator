            ...
            if self.conf.CAPTURE_COLLISION_MODEL_ENABLED:
                ...
                if p.sensedByN[self.nodeid]:
                    ...
                    if p.phyLostAtN[self.nodeid]:
                        logger.debug(f"{self.env.now:.3f} Node {self.nodeid} lost packet {p.seq} to weak-link PHY errors.")
                        continue
                    p.receivedAtN[self.nodeid] = True
                    logger.debug(f"{self.env.now:.3f} Node {self.nodeid} received packet {p.seq} with delay {round(self.env.now - p.genTime, 2)}")
